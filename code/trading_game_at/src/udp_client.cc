...
bool UDPClient::Poll()
{
	...
	char buffer[1024];
	const auto bytesReceived = ::recvfrom(mSocket, buffer, sizeof(buffer) - 1, 0, (struct sockaddr *)&clientAddress, &addressLength);
	if (bytesReceived < 0)
	{
	...
	const std::experimental::string_view message{buffer, static_cast<std::size_t>(bytesReceived)};
	mHandler.OnMulticastMessage(message);
